...
create_template() {
	gcloud compute instance-templates create $1 --custom-cpu="8" --custom-memory="61" --custom-extensions --image="cold-cluster-instance-recreated-image-new-v11" --boot-disk-type="pd-ssd" --boot-disk-size="50" --local-ssd interface=NVMe --local-ssd interface=NVMe --local-ssd interface=NVMe --local-ssd interface=NVMe --network="zapr-vpc-network" --subnet="private-1" --tags="allow-ssh" --preemptible --metadata-from-file startup-script="./cold-startup-script.sh"  --scopes="monitoring,pubsub,storage-rw" --metadata partition=$2 --region=asia-south1
}

...
# THIS SCRIPT IS FOR PROVISIONING COLD CLUSTER VMs
...
create_template_existing_disk() {
        gcloud compute instance-templates create $1 --custom-cpu="8" --custom-memory="61" --custom-extensions --image="cold-cluster-instance-recreated-image-new-v18" --boot-disk-type="pd-ssd" --boot-disk-size="50" --disk=name=cold-disk-0$i --network="zapr-vpc-network" --subnet="private-1" --tags="allow-ssh" $3 --metadata-from-file startup-script="./cold-startup-script-existing-disk.sh"  --scopes="monitoring,pubsub,storage-rw" --metadata partition=$2 --region=asia-south1
#        --disk=name=cold-disk-$i  use this instead of create-disk flag once all disks have been created
}

create_template_new_disk() {
        gcloud compute instance-templates create $1 --custom-cpu="8" --custom-memory="61" --custom-extensions --image="cold-cluster-instance-recreated-image-new-v18" --boot-disk-type="pd-ssd" --boot-disk-size="50" --create-disk=mode=rw,size=500,type=pd-ssd,name=cold-disk-0$i,device-name=persistent-disk-0$1 --network="zapr-vpc-network" --subnet="private-1" --tags="allow-ssh" $3 --metadata-from-file startup-script="./cold-startup-script.sh"  --scopes="monitoring,pubsub,storage-rw" --metadata partition=$2 --region=asia-south1
#        --disk=name=cold-disk-$i  use this instead of create-disk flag once all disks have been created
}
...
main(){
        for ((i=$1; i<=$2; i++)); do
          ...
          then
            ...
            create_template_new_disk cold-temp-$i $i $3
          else
            echo "Caution: This attached existing PD-SSD of size 500 gb"
            create_template_existing_disk cold-temp-$i $i $3
          fi
          if (( $i % 3 == 1 ))
          then
            create_instance_group cold-group-$i cold-temp-$i $i asia-south1-a
            create_backend_service cold-backend-$i $i
            attach_backend cold-backend-$i cold-group-$i asia-south1-a
          elif (( $i % 3 == 2 ))
          then
            create_instance_group cold-group-$i cold-temp-$i $i asia-south1-b
            create_backend_service cold-backend-$i $i
            attach_backend cold-backend-$i cold-group-$i asia-south1-b
          else (( $i % 3 == 0 ))
            create_instance_group cold-group-$i cold-temp-$i $i asia-south1-c
            create_backend_service cold-backend-$i $i
            attach_backend cold-backend-$i cold-group-$i asia-south1-c
          fi       
    done
}
